...
* загрузка и сохранение адресных книг следующих форматов:
  ...
  - CSV (поддерживаются профили для некоторых моделей телефонов Explay, программ OSMO и Sylpheed, а также универсальный профиль, сохраняющий всю информацию о контакте);
...
По умолчанию программа сохраняет полное/форматированное имя в файл VCF, даже если в контакте оно не задано. Более того, некоторые программы, например, почтовый клиент Sylpheed, не распознают контакты, не содержащие тег FN. Однако это поведение можно изменить для записи  на некоторые старые устройства (например, телефон SonyEricsson W300i не сохранял FN). Поведение задаётся флажком "Писать тег FN, если полного имени нет" на вкладке "Сохранение".
...
Стандарт vCard предусматривает тег CATEGORIES для хранения информации о группах контактов. К сожалению, большинство андроид-устройств при экспорте
адресной книги либо не используют эту возможность, либо пишут в этот тег только служебные категории (например, My Contacts). Группы, определённые
пользователем (например, Семья, Коллеги), в этих случаях не сохраняются в файл VCF.
...
* использование сетевых адресных книг, таких как [Google Contacts](https://contacts.google.com/) или ownCloud/Nextcloud
с последующей выгрузкой контактов в файл VCF.
...
профили для ряда моделей телефонов Explay. Кроме того, поддерживаются разновидности CSV, сохраняемые программами
[Osmo](http://clayo.org/osmo/) и Sylpheed.
...
Нажатие Ctrl+Q или выбор **Вид -> Быстрый просмотр** включает режим быстрого 
просмотра: на второй панели отображается подробная информация о контакте, 
выбранном на текущей (только если выбран один контакт).
...
Если перед этим отметить флажок **Удалить правый контакт после слияния**, программа оставит объединённый контакт
только в левой адресной книге, а в правой удалит его.
