...
* загрузка адресных книг форматов VCF (поддерживаются vCard 2.1 и 3.0, а также возможность загрузки каталога,
содержащего множество файлов VCF, как единой адресной книги), UDX (Philips Xenium), MPB (резервная копия MyPhoneExplorer)
и сохранение адресных книг в указанных форматах;
...
перевод телефонных номеров в международный формат (например, замена 8 на +7 для России, аналогичные преобразования
для некоторых других стран), разделение контакта по телефонам для экспорта на SIM-карту, слияние контактов,
...
который DoubleContact тоже умеет открывать. Некоторые телефоны, например, 
Чтобы экспортировать сразу несколько контактов

TODO меню телефона и хитрость с открытием нескольких
...
Таким же образом можно загрузить контакты из файла резервной копии программы MyPhoneExplorer (расширение MPB). В этом случае
загружаются не только контакты, но и история вызовов, заметки, события и SMS. DoubleContact не поддерживает эти данные, но
...
Запись в файл MPB можно производить только, если исходный файл также имел формат MPB. Это сделано потому, что файл MPB содержит не только контакты,
но и историю вызовов и SMS (данные, не поддерживаемые в VCF и UDX). Если сохранить MPB в VCF, а потом снова в VCF, история вызовов и SMS будут
утеряны!
...
* "Задать адрес" - позволяет ввести или отредактировать ссылку на фото в отдельном окне. Если
в контакте была фотография, она будет удалена;
...
для некоторых стран. Для Белоруссии эта операция заменяет префикс 8 на код страны +375, для России - 8 на +7,
для Украины - 0 на +380.
...
ВНИМАНИЕ! Автоматический выбор страны невозможен, поскольку для разных стран одному и тому же национальному префиксу
могут соответствовать разные международные. Например, для России 8 переводится в +7, для Белоруссии TODO тот же 8 переводится в +375.
Попытка перевести в международный формат сразу все контакты может привести к порче телефонных номеров.
Поэтому если в телефонной книге у разных контактов присутствует национальная запись как для российских, так и для белорусских
телефонов, сначала выберите и переведите белорусские номера, потом российские, или наоборот. Это, разумеется,
очень редкий случай, но он может возникнуть после слияния двух адресных книг, одна из которых изначально
заполнялась в России, а другая в Белоруссии. Особо тяжёлый случай - когда телефоны с "национальной" записью
для Белоруссии и России присутствуют в одном контакте. Эту ситуацию можно исправить только вручную. К счастью,
в жизни она вряд ли возникнет.
...
форматированные имена, псевдонимы и организации. Таким образом, проверка выполняется от более надёжных критериев
...
(подсвечена зелёным либр жёлтым цветом), парная запись также будет выделена. Если после этого вызвать
...
таких контактов, вероятно, требуется разделить или переставить имена (см. раздел "Групповые операции").
...
Багтрекер программы располагается на гитхабе по адресу TODO Если Вы не умеете работать с багтрекером, но нашли
важную ошибку, вы можете написать мне по электронной почте pub@zvyozdochkin.ru. Пожалуйста, пишите письма на английском или русском
языках.
...
В настоящее время автор работает над поддержкой формата CSV (comma-separated values). Нет единого стандарта, как
контакты должны сохраняться в формате CSV. Если Вы заинтересованы в поддержке этого формата, пришлите пример файла CSV,
созданного вашим устройством или программой, или его фрагмент. Если Вы прислали фрагмент, пожалуйста, проверьте,
...
который программа не переведена - присоединяйтесь. TODO: адрес.
...
* службам GitHub, Travis CI и Coverity Scan - за одноимённые (TODO слева фирмы, справа названия) службы, помогающие писать программы и доносить их до людей.
