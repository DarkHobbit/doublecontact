...
из этого типа файлов контакты можно только читать, запись не поддерживается. Это сделано потому, что файл MPB содержит не только контакты,
но и историю вызовов и SMS (данные, не поддерживаемые DoubleContact).
??? Замеченный глюк в MPB: если поле NOTE (описание контакта) содержало перевод строки, появляется фальшивое ENCODING=QUOTED-PRINTABLE
ПРОВЕРИТЬ
